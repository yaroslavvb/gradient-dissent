...
/work/vit-initialization-audit. This is a numerical/provenance audit, not a GPU job.
"""
import hashlib
import json
...
ROOT=Path('/work/vit-initialization-audit')
...
for seed in (1000,2000,2001,2002):
    ...
    reports.append({'task':'vit_cifar100','seed':seed,'state_hashes':[digest_state(aa),digest_state(bb)],'parameters':total,'changed_elements':changed,'max_absolute_difference':maximum,'relative_l2_difference':relative,'passed':maximum<=THRESHOLDS['max_absolute_difference'] and relative<=THRESHOLDS['relative_l2_difference'],'tensors':rows})
    del aa,bb,ad,bd,d
