...
app = modal.App('gradient-dissent-mnist-layerdrop-hypotheses')
...
def train(spec):
    sys.path.insert(0, '/opt/ciresan_stochastic_depth')
    if spec.get('mode','audit') == 'routing':
        from layerdrop_hypotheses.routing_benchmark import run
    else:
        from layerdrop_hypotheses.audit import run
    ...
    def monitor_budget():
        while not stop_monitor.is_set():
            try:
                ...
                (out / 'hypotheses' / 'billing-latest.json').write_text(json.dumps({'metered_cost_usd':cost,'queried_unix':time.time()}, indent=2)+'\n')
                if cost >= 24:
                    budget_halted.set()
