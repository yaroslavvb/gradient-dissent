...
All 18 completed telemetry runs are now uploaded to [yaroslavvb/gradient-dissent](https://wandb.ai/yaroslavvb/gradient-dissent) and verified through the W&B API. The [upload registry](../results/telemetry/wandb-upload-manifest.json) contains actual run URLs, source hashes, per-run row/cell counts and verification timestamps. W&B SDK 0.30.0 replays the saved measurements after training, with no GPU calls or new optimization. The JSONL path remains available without authentication.
...
The [curated W&B report](https://wandb.ai/yaroslavvb/gradient-dissent/reports/Ciresan-MNIST:-measured-training-curves-·-2026-09-09--VmlldzoxNzkwNDAzNA==) contains eight charts: controlled train/validation cross-entropy and accuracy, two fixed-probe gradient statistics, and separate official-test target replay views. Charts use saved training time or measured run elapsed time, with one unsmoothed trace per seed. Its three run sets are restricted to the exact 18 verified source IDs. The saved configuration was reloaded and checked through the API; W&B access was verified using the authenticated account. [Report builder and verification guide](WANDB_REPORT.md) documents the locked `wandb-workspaces` package and repeatable publication command.

## Verification

Fifteen exporter CPU tests pass using a fake SDK, with no network calls. They cover source immutability, clock preservation, absent-axis refusal, units and test/validation aliases, finite values, safe configuration, new run IDs, JSONL checksums, historical-project rejection, metadata controls and custom-axis logging. The actual recorded 837-metric row also passes: the sanity bound is 2,048 total fields to include its additional scalar diagnostics, with no truncation. All 18 completed reruns were exported as JSONL, and every original finite metric, recorded clock, source hash and JSONL hash was checked. A real SDK offline smoke test produced a native W&B file from a complete 22-row run in a temporary directory; it uploaded nothing and the temporary files were removed. The subsequent online upload verifies every saved scientific history cell and frozen endpoint/timing summary against server readback. A regression test also covers SDK 0.30 wrapping a missing-run exception inside `CommError`; unrelated API errors are not treated as absence. Run the tests with:
...
The registry exposes the SDK-returned URL only after the public API retrieves the actual finished run, every expected scalar/clock cell, all source hashes, and frozen endpoint summaries. `scan_history` is used without key intersection or cached history so sparse metrics are retained. Ten additional CPU tests exercise prefix conflicts, completion and URL gating, endpoint units, and the exact 18-source registry. No model weights or checkpoints are uploaded. [Public API](https://docs.wandb.ai/models/ref/python/public-api/api), [run-history interface](https://docs.wandb.ai/models/ref/python/public-api/run).

The user selected `yaroslavvb/gradient-dissent` during upload. Fifteen finished scalar imports were moved there using W&B’s server-side `moveRuns` operation; none of these IDs remained in the former project. The three baseline imports were then completed in the requested project. Every one of the 18 runs was fully verified at its final URL. The [migration receipt](../results/telemetry/wandb-project-migration.json) records the exact IDs and destination; no model artifacts or training were involved.
