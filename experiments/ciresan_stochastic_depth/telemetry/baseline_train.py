...
def run(spec, root, invocation_start=None, progress_commit=None):
    ...
    root=Path(root)
    run_dir=root/'runs'/spec['run_id'];run_dir.mkdir(parents=True,exist_ok=True)
    ...
    result={'run_id':spec['run_id'],'spec':spec,'dataset':{'train_n':60000,'test_n':10000,'files':files},
            'source_sha256':{name:hashlib.sha256((Path(__file__).parents[1]/name).read_bytes()).hexdigest() for name in ['telemetry/baseline_train.py','telemetry/recorder.py','telemetry/metrics.py','optimization/kernels.py','model_data.py']},
    ...
    if spec.get('kind')=='microbench':
        ...
        recorder=Recorder(model,spec,run_dir,{'train_x':x,'train_y':y,'test_x':tx,'test_y':ty},start,helper=None)
        ...
        for epoch in range(spec.get('max_epochs',100)):
            ...
            if (epoch+1)%spec.get('eval_every',1)==0:
                ...
                history.append(row)
                recorder.capture(epoch+1,training_seconds,elapsed,legacy_row=row)
                ...
                if threshold is None and score['errors']<=137:
                    threshold=row.copy()
                    threshold['run_wall_seconds_with_telemetry']=time.perf_counter()-start
        ...
        result['trained_final_parameters_sha256']=hashlib.sha256(b''.join(p.detach().cpu().numpy().tobytes() for p in model.parameters())).hexdigest()
