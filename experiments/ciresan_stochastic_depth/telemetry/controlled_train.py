...
def _source_hashes():
    names=("telemetry/controlled_train.py","telemetry/recorder.py","telemetry/metrics.py","train_optimized.py","model_data.py","optimization/stochastic_graph.py",
...
def run(spec, root, progress_commit=None):
    ...
    recorder=Recorder(model,spec,run_dir,tensors,start_time,helper=helper)
    ...
    for epoch in range(spec["epochs"]):
        ...
        if epoch==0 or (epoch+1)%eval_every==0 or epoch==spec["epochs"]-1:
            ...
            recorder.capture(epoch+1,train_seconds,elapsed,legacy_row=history[-1])
    ...
    if spec["stage"]=="evaluate" and not failed:
        try:
            ...
            final_parameter_hash=hashlib.sha256(b"".join(p.detach().cpu().numpy().tobytes() for p in model.parameters())).hexdigest()
    ...
    result["trained_final_parameters_sha256"]=locals().get("final_parameter_hash")
    recorder.finalize(result)
    result["final_parameters_sha256"]=hashlib.sha256(b"".join(p.detach().cpu().numpy().tobytes() for p in model.parameters())).hexdigest()
