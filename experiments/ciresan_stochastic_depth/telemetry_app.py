...
Run with: modal run -e gradient-dissent-ciresan experiments/ciresan_stochastic_depth/modal_app.py --stage prep
...
VOLUME = 'gradient-dissent-ciresan-20260909'
ROOT = '/work'
...
CAP = 30.0
RESERVATION_CEILING = 24.0  # Leave $6 for build, egress, billing lag and interruption overhead.
app = modal.App('gradient-dissent-ciresan-telemetry')
image = (modal.Image.debian_slim(python_version='3.11')
         .apt_install('build-essential')
         .pip_install('torch==2.14.0', 'numpy==2.2.6')
         .env({'OMP_NUM_THREADS': '2', 'MKL_NUM_THREADS': '2', 'PYTHONUNBUFFERED': '1'})
         .add_local_dir(HERE, remote_path='/opt/ciresan_stochastic_depth',
                        ignore=['results', 'data', 'checkpoints', '.venv', '__pycache__']))
volume = modal.Volume.from_name(VOLUME, create_if_missing=True)

@app.function(image=image, cpu=(2, 2), memory=(8192, 8192), timeout=180,
              startup_timeout=90, retries=0, max_containers=1, scaledown_window=2,
              volumes={ROOT: volume})
def prepare():
    sys.path.insert(0, '/opt/ciresan_stochastic_depth')
    from model_data import load_mnist
    start = time.time()
    data = load_mnist(Path(ROOT) / 'data', include_test=False)
    result = {'dataset': data['metadata'], 'seconds': time.time() - start}
    (Path(ROOT) / 'dataset-manifest.json').write_text(json.dumps(result, indent=2))
    volume.commit()
    return result

@app.function(image=image, gpu='A100', cpu=(2, 2), memory=(8192, 8192),
              timeout=480, startup_timeout=90, retries=0,
              max_containers=4, scaledown_window=2, volumes={ROOT: volume})
def train(spec):
    sys.path.insert(0, '/opt/ciresan_stochastic_depth')
    from telemetry.run import run
...
def _reserve_unlocked(jobs, kind):
    ...
    """
    out = HERE / 'results' / 'telemetry'; out.mkdir(parents=True,exist_ok=True)
    ...
    ledger = json.loads(path.read_text()) if path.exists() else {
        ...
        'note': 'Reservations are conservative upper estimates, not invoices. Extra $6 buffer covers image build, egress and interruptions.',
        'reservations': []}
    new = []
    for job in jobs:
    ...
    opening=json.loads((out/'budget-opening.json').read_text())
    ledger['opening_metered_usd']=opening['metered_opening_usd']
    ledger['legacy_ledger_sha256']=opening['legacy_ledger_sha256']
    total = opening['metered_opening_usd'] + sum(r['reserved_usd'] for r in ledger['reservations'] + new)
...
def reserve(jobs, kind):
    lock = HERE / 'results' / 'telemetry' / '.budget.lock'
...
def launch(stage: str = 'prep', manifest: str = ''):
    out = HERE / 'results' / 'telemetry'; out.mkdir(parents=True,exist_ok=True)
    if stage == 'prep':
        reserve([{'run_id': 'data-preparation-v1', 'timeout_seconds': 300}], 'prep')
        result = prepare.with_options(timeout=300).remote()
    ...
    def monitor_budget():
        while not stop_monitor.is_set():
            try:
                ...
                if cost >= 24:
                    budget_halted.set()
                    print('BUDGET STOP: environment metered usage reached $24; cancelling outstanding calls.', flush=True)
    ...
    def execute(spec):
        ...
        try:
            ...
            call = train.with_options(timeout=spec['timeout_seconds'],gpu=spec.get('gpu','A100')).spawn(spec)
            calls.append(call)
