...
if not modal.is_local():
    # Modal auto-mounts this wrapper at /root; its sibling source directory is
    # supplied by the shared image at /opt, before function-module hydration.
...
MANIFEST = OUT / 'manifest-v2.json'
FREEZE = OUT / 'source-freeze-v2.json'
GUARD_USD = 24.0
...
def validate_jobs(jobs):
    ...
    for spec in jobs:
        if (spec.get('run_id') != f"full-depth-s{spec['seed']}-v2"
...
def launch():
    ...
    opening_path = OUT / 'budget-opening-v2.json'
    ...
    write_json(OUT / 'reservation-receipt-v2.json', {
    ...
    write_json(OUT / 'billing-at-call-completion-v2.json', closing)
