...
app = modal.App('gradient-dissent-ciresan-optimization')
...
@app.function(image=image, gpu='A100', cpu=(2, 2), memory=(8192, 8192),
              ...
              max_containers=6, scaledown_window=2, volumes={ROOT: volume})
def train(spec):
    sys.path.insert(0, '/opt/ciresan_stochastic_depth')
    invocation_start = time.perf_counter()
    from optimization.benchmark import run
    result = run(spec, Path(ROOT), invocation_start=invocation_start, progress_commit=volume.commit)
    volume.commit()
