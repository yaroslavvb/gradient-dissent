...
**The layer-scope selector now offers all six learned layers or the original four middle branches.** In the original scope, unchecking everything still leaves the learned feature layer (stem) and classifier (head). The stem maps 784 pixels to 2,500 ReLU features; crops retain the first 500, and the classifier produces ten digit scores. This trained shallow predictor uses **16.42% of the counted affine work**. In the extended scope, dropping the stem inserts an untrained zero-padded pixel representation. Dropping the classifier means **no prediction**: coverage and correct outputs fall to zero, while classification accuracy is undefined. Uniform random guessing would average 10%, not zero.
...
The method selector chooses already-trained weights. **Fixed inference** uses the checked layers for every image. **Random inference** makes checked layers eligible for independent dropout at the slider's probability; unchecked layers always remain. Its scores are exact expectations over single mask draws, not a prediction ensemble. Retained branches use gain one (no rescaling), and neither mode retrains the model. A validation-selected checkpoint can occur before the decreasing training schedule reaches zero.
...
The network has widths 784→2500→2000→1500→1000→500→10 and 11,972,510 parameters. Its four eligible middle transitions have a prefix-crop shortcut followed by ReLU. The learned stem and classifier remain compulsory in this original four-branch audit; the separately reported six-layer follow-up removes that restriction. Cropping loses coordinates and widths change, so this architecture is not an invertible, equal-dimensional flow. The main models use normalized inputs and linear output logits; original raw-input/ReLU-output fidelity runs are outside this audit.
...
## Measured A100 latency: static masks win this implementation
...
|---|---:|---:|---:|---:|---:|---:|
| residual / 101 | 4.868 | 4.075 | 13.637 | 9.610 | 98.32% | 98.21% |
| sd_annealed / 101 | 4.830 | 2.697 | 13.589 | 9.376 | 97.97% | 98.11% |
| sd_constant / 101 | 4.727 | 3.583 | 15.374 | 11.604 | 97.41% | 97.60% |
| residual / 102 | 4.832 | 4.049 | 15.536 | 11.569 | 98.11% | 97.96% |
| sd_annealed / 102 | 4.832 | 4.027 | 12.583 | 9.797 | 97.58% | 97.49% |
| sd_constant / 102 | 4.461 | 2.335 | 9.242 | 6.280 | 97.17% | 96.94% |
| residual / 103 | 4.828 | 3.449 | 14.610 | 11.184 | 97.70% | 97.90% |
| sd_annealed / 103 | 4.816 | 2.690 | 11.062 | 7.887 | 97.82% | 97.82% |
| sd_constant / 103 | 4.827 | 2.506 | 11.237 | 8.478 | 97.68% | 97.83% |

Across the nine per-model medians, dense inference takes **4.83 ms**, static masks **3.45 ms**, and image-dependent routing **13.59 ms**. The router is **2.07–3.25× slower** than dense inference on every model. Static masks are **1.19–1.93× faster**, at the accuracy/cost points shown above. These masks were selected using validation; several exceed the empirical 0.2pp error tolerance on test, so speed is not an unconditional quality-preservation claim.
...
[Raw timing run](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypothesis-routing-v1.json) · [Compact timing summary](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypotheses/routing-summary.json) · [Timing implementation](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/layerdrop_hypotheses/routing_benchmark.py)

## Original four-branch audit: compute cost and execution

| Audit seed | GPU | Function execution seconds | Dispatch to result seconds |
|---|---|---:|---:|
| 101 | NVIDIA A100-SXM4-40GB | 22.64 | 30.39 |
| 102 | NVIDIA A100-SXM4-40GB | 25.52 | 34.92 |
| 103 | NVIDIA A100-SXM4-40GB | 27.28 | 37.63 |

The fourth routing job took 17.55 seconds inside the function and 30.13 seconds dispatch-to-result. All four paid calls completed; all cloud apps are stopped. No neural training was performed. The shared MNIST environment’s metered snapshot rose from **$3.97677 to $4.07871**, an increase of about **$0.10**. Snapshot checked 2026-09-09T23:27:08.442662+00:00; this is metering, not a final invoice.

The original **$30** cap still applies. Immutable worst-case reservations rose from $23.10357 to **$23.88100**, including $0.77743 for these four 180-second A100 allocations plus startup/idle allowances. Old reservations were not released or reset. Reservations and actual metering are different quantities.

[Billing snapshot](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypotheses/billing-latest.json) · [Shared reservation ledger](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/budget-ledger.json) · [Stopped-app check](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypotheses/apps-stopped.json)
...
[Frozen experimental protocol](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/layerdrop_hypotheses/PROTOCOL.md) · [Pre-audit hashes/specifications](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/layerdrop_hypotheses/audit-freeze.json) · [Frozen policies](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypotheses/policy-manifest.json) · [Full analysis](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypotheses/analysis.json) · [Independent verification](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypotheses/independent-check.json)

[Fable’s argument: mathematical audit](https://github.com/yaroslavvb/gradient-dissent/blob/main/research/fable-layerdrop-audit.md) · [Executed counterexamples](https://github.com/yaroslavvb/gradient-dissent/blob/main/experiments/ciresan_stochastic_depth/results/hypotheses/theory-counterexamples.json)
