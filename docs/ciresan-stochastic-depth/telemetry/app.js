...
function scalarPoints(r,key){return (r.telemetry_rows||[]).filter(t=>Object.hasOwn(t.metrics,key)||Object.hasOwn(t.metrics,'train/accuracy_pct')).map(t=>({x:t[$('axis').value],y:t.metrics[key],epoch:t.epoch}));}
function renderMetric(){const key=$('metric').value,series=selected().map(r=>trace(r,scalarPoints(r,key)));chart('metric-chart',series,{xlabel:tx($('axis').value),ylabel:key,log:$('metric-log').checked});const layer=key.match(/^layer-(\d)/)?.[1]||'1',normkey=`layer-${layer}/probe/grad_l2`;$('norm-heading').textContent=`Layer ${layer} · mean gradient magnitude`;chart('gradient-norm',selected().map(r=>trace(r,scalarPoints(r,normkey))),{xlabel:tx($('axis').value),ylabel:'Mean probe weight-gradient L2 norm',log:true});$('metric-note').textContent=key?`${key}. ${metricKeys.length} recorded scalar keys available. Missing values break the line; zero or negative values are omitted on a log axis. Probe gradient moments use ordinary FP32 autograd; the later curvature pass uses stabilized float64 derivatives.`:'No metric matches this search.';}
